...
import { appendFileSync, createReadStream, existsSync, lstatSync, readdirSync, readFileSync, unlinkSync, writeFileSync, } from 'fs';
...
(function (FileType) {
    ...
    FileType["TextJavascript"] = "text/javascript";
})(FileType || (FileType = {}));
export class UploadSourceMapsHandler {
    ...
    env = {};
    sessionId = null;
    constructor(path, distName) {
        this.path = path;
        this.distName = distName;
        this.fullpath = join(path, distName ?? '');
    }
    async startUploadSession(version) {
        try {
            const res = await fetch(`${Config.BB_API_URL}/sourcemaps/session${version ? `?version=${version}` : ''}`, {
                method: 'GET',
                headers: {
            ...
            try {
                ...
                if (res.status !== 201) {
                    if (!data.errors[0]) {
                        throw new Error('Unexpected error. please contact us at support@byteboost.io');
                    }
    ...
    compileSourceMapPathsList(path = this.fullpath) {
        try {
    ...
    async tagFilesWithDebugInfo() {
        ...
        for (const sourceMapPath of this.mapFilePaths) {
            ...
            for (const filePath of fileGroup) {
                ...
                try {
                    if ((await this.readLastLine(filePath)).includes('//# bbDebugId') &&
                        !Config.BB_DEBUG) {
                        continue;
                    }
                }
                ...
                if (filePath.endsWith('.map')) {
                    const sourceMapContent = this.parseJSON(readFileSync(filePath, 'utf-8'));
                    if (sourceMapContent) {
    ...
    async uploadSourcemaps() {
        ...
        if (res.status !== 201) {
            try {
                ...
                console.log(`[Byteboost] Unexpected error. Please contact us at support@byteboost.io`);
            }
            return;
        }
        return;
    ...
    loadEnvironmentVariables() {
        try {
            ...
            console.log(`[Byteboost] Couldn't find the .env file in your project root path ${this.path}`);
            console.log(`
      Env format:
