...
const getPackageVersion = (path, usePackageVersion) => {
...
export const UploadSourcemapsCommand = new Command()
    ...
    .option('-v, --packageJSONVersion', 'Use the version from package.json. If this is not defined we will automatically generate an unique id for you')
    .option('--dist <dist>', 'Name of the distribution folder, eg build or dist')
    .option('-c, --cleanup', 'Remove sourcemaps after uploading. This is helpful if you are running for example nextjs and dont want your sourcemaps to be public.')
    ...
    if (!isValidJsDir) {
        console.warn(`[Byteboost] The path ${path} is not a valid JS project directory. Couldn't find package.json`);
        return;
    }
    const success = handler.loadEnvironmentVariables();
    if (success !== true) {
        if (success !== false)
            console.log(`[Byteboost] ${success}`);
        return;
    }
    try {
        const sessionId = await handler.startUploadSession(getPackageVersion(path, options.packageJSONVersion));
        handler.sessionId = sessionId;
        handler.compileSourceMapPathsList();
        if (!handler.mapFilePaths[0]) {
            console.log(`[Byteboost] No sourcemaps found in path ${handler.fullpath}`);
            return;
        }
        ...
        console.log(`[Byteboost] Uploaded sourcemaps in ${(Date.now() - startTime) / 1000} seconds`);
    }
    catch (err) {
